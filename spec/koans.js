...
describe("the JavaScript language", function() {
  ...
  describe("has different types and operators", function() {
    ...
    it("may contain functions inside arrays", function() {
      ...
      expect(arr[2](1)).toEqual(3 + 1); // @RECAP
    });
    ...
    it("can't compare arrays", function() {
      ...
      expect(a == b).toBeFalsy();  // Truthy or Falsy => arrays can't be compared
  ...
  describe("considers functions as first class citizens", function() {
    ...
    it("may return anything", function() {
      ...
      expect(result[1]).toEqual(4); // => 2 * 2 = 4
    });

    it("may return arrays that contains functions and so on", function() {
      function example() {
         // function's position = 0; 1 = arg
         const someArr = [function(arg) {
           // Omit first element, pick up second element at index 1 and multiply arg with 10
           return [, arg * 10];
        } ];
         return someArr;
      }
    ...
    it("matters, the declaration order when they are anonymous", function() {
      ...
      expect(exampleA()).toEqual(1); // Change callback function order and pass in arg = 1
    });
    ...
    it("anonymous functions are anonymous", function() {
      ...
      expect(typeof (z)).toEqual('undefined'); // @REMEMBER z => ReferenceError: z is not defined; x => [Function: z]
      expect(x()).toEqual(1);
    });
    ...
    it("defines a pure function when there are no free variables", function() {
      function external() {
        ...
        return internal(4,4); // a = 4, b = 4, c = 1 => 4 + 4 + 1 = 9;
      }
